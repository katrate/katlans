...
# Infix built-in operators: x Speeks "word", lst Lhas val, etc.
INFIX_BUILTINS = {
    "Speeks", "Sfront", "Sback",   # string checks
    "Lhas",                          # list contains
    "Ahas",                          # dict key check
    "SThas",                         # set contains
    "Zhas",                          # tuple contains
...
class Parser:
    ...
    def _parse_stmt(self):
        ...
        # testrun() ::  or  testrun <name> ::
        if tok.type == TT.TESTRUN:
            return self._parse_testrun()

        # testerr <fxn name, "ErrorType"> ::
        if tok.type == TT.TESTERR:
            return self._parse_testerr()

        # Identifier-led: assignment or expression
        if tok.type in (TT.IDENT, TT.SELF):
    ...
    def _parse_typed_decl(self) -> VarDecl:
        ...
        # If next token is LBRACE or a literal-starting token, parse as literal
        # Otherwise parse as general expression (e.g. function call result)
        if dtype == "lt":
            if self._check(TT.DCOLON):   # empty list
                value = ListLiteral(elements=[], line=tok.line, col=tok.col)
            elif self._check(TT.IDENT) and self._peek().type == TT.LPAREN:
                value = self._parse_expr()   # e.g. lt x = someFunc() ::
            else:
                value = self._parse_list_literal()
        elif dtype == "zl":
            if self._check(TT.IDENT) and self._peek().type == TT.LPAREN:
            ...
            if self._check(TT.LBRACE):
                ...
                value = self._parse_expr()   # e.g. ad x = jsread("f.json") ::
        else:
            value = self._parse_expr()
    ...
    def _parse_func_call_stmt(self):
        tok = self._advance()   # fxn
        # Could be:  fxn add <args>  OR  fxn dog.speak <args>
        name_tok = self._expect(TT.IDENT, "expected function name")
        obj  = None
        name = name_tok.value

        if self._check(TT.DOT):        # fxn dog.speak
            self._advance()
            method_tok = self._expect(TT.IDENT, "expected method name")
            obj  = name
            name = method_tok.value

        args = []
        if self._check(TT.LT_OP):
            args = self._read_angle_expr_list()
        self._expect_dcolon()
        return FuncCall(name=name, args=args, obj=obj,
                        line=tok.line, col=tok.col)
    ...
    def _parse_test_decl(self):
        ...
        self._expect(TT.BLOCK_OPEN, "expected '|>'")
        body = self._parse_block()
        return TestDecl(name=name, body=body, line=tok.line, col=tok.col)

    def _parse_testcheck(self):
        tok = self._advance()   # testcheck
        self._expect(TT.LT_OP); self._angle_depth += 1
        # Capture the raw source of the expression for output
        start_pos = self.pos
        expr = self._parse_expr()
        # Build a rough string rep
        end_pos = self.pos
        expr_str = " ".join(str(t.value) for t in self.tokens[start_pos:end_pos]
                            if t.type != TT.EOF)
        self._angle_depth -= 1; self._expect(TT.GT_OP)
        self._expect_dcolon()
        return TestCheck(expr=expr, expr_str=expr_str,
                         line=tok.line, col=tok.col)
    ...
    def _parse_testrun(self):
        ...
        if self._check(TT.LT_OP):
            ...
            self._advance(); self._expect(TT.RPAREN)
        self._expect_dcolon()
        return TestRun(name=name, line=tok.line, col=tok.col)

    def _parse_testerr(self):
        tok = self._advance()   # testerr
        args = self._read_angle_expr_list()
        self._expect_dcolon()
        func = args[0] if args else None
        err_type = args[1].value if len(args) > 1 else ""
        return TestErr(func=func, error_type=err_type,
                       line=tok.line, col=tok.col)
    ...
    def _parse_ident_stmt(self):
        ...
        if (self._check(TT.IDENT) and self._peek().type == TT.IDENT
                and self._peek().value not in ("dec",)):
            # peek ahead past 2 idents to see if there's = cls
            saved = self.pos
            self._advance(); self._advance()
            is_cls_inst = self._check(TT.ASSIGN)
            self.pos = saved
            if is_cls_inst:
                ...
                if self._check(TT.LT_OP):
                    args = self._read_angle_expr_list()
                self._expect_dcolon()
                return ClassInstantiate(cls_name=inst_cls, var_name=var_name,
                                        args=args, line=tok.line, col=tok.col)

        # Built-in call with angle brackets as statement: Ladd <lst, val> ::
        ANGLE_BUILTINS = {
            # List/Dict mutations
            "Ladd","Laddall","Linsert","Ldrop","Lclear","Lsort","Lflip",
            "Aset","Aadd","Adrop","Aupdate","Aclear",
            # File I/O
            "fwrite","fadd","fdel","fmkdir","frmdir","frename","fcopy",
            "fwritelines",
            # JSON / CSV / XML write
            "jswrite","csvwrite",
            # OS
            "oscd","osclipset","osnoti","osopen","oshalt","onset",
            # CLI output
            ...
            # DateTime
            "dtwait","dtadd","dtsub",
            # Networking (angle-bracket call style)
            "netget","netpost","netput","netpatch","netdel",
            "netjson","netdl","netdns","netping",
            # Data structures mutations
            "dspush","dspop","dsenqueue","dsdequeue",
            "dsadd","dsrem","dsllpush","dsllpop",
            "dstadd","dsgedge","dshpush","dqdpushl","dqdpushr",
            "dsmatset","dsmatprint",
            # Concurrency
            "cxpush","cxpop","cxsend","cxjoin","cxstop",
            "cxacquire","cxrelease","cxabort","cxsubmit",
            "cxinc","cxdec",
        }
        if (self._check(TT.IDENT) and
                self._peek().type == TT.LT_OP and
                self._cur().value in ANGLE_BUILTINS):
            fn_tok = self._advance()
            args   = self._read_angle_expr_list()
            self._expect_dcolon()
            return BuiltinCall(func=fn_tok.value, args=args,
                               line=fn_tok.line, col=fn_tok.col)

        # x = expr  /  self.x = expr  /  x.member = expr
    ...
    def _parse_template_str(self, tok: Token):
        """
        "Hello {name}, you are {age}!"
        → concat(concat(concat("Hello ", S(name)), ", you are "), S(age), "!")
        Splits on {expr} groups, re-lexes and re-parses each embedded expression.
        """
        raw   = tok.value
        parts = re.split(r'\{([^}]*)\}', raw)
        nodes = []
        for i, part in enumerate(parts):
            if i % 2 == 0:          # literal text segment
                if part:
                    nodes.append(StrLiteral(value=part,
                                            line=tok.line, col=tok.col))
            else:                   # embedded expression
                sub_lex    = Lexer(part.strip(), self.filename)
                sub_tokens = sub_lex.tokenise()
                sub_parser = Parser(sub_tokens, self.filename)
                sub_parser._angle_depth = self._angle_depth
                expr = sub_parser._parse_expr()
                # wrap in S() so any type prints as string
                nodes.append(BuiltinCall(func="S", args=[expr],
                                         line=tok.line, col=tok.col))

        if not nodes:
            return StrLiteral(value=raw, line=tok.line, col=tok.col)

        result = nodes[0]
        for node in nodes[1:]:
            result = BinOp(op="++", left=result, right=node,
                           line=tok.line, col=tok.col)
        return result
        ...
        while True:
            if self._check(TT.DOT):
                ...
                node = IndexAccess(obj=node, index=idx, end=end, step=step,
                  ...
                  self._cur().value in INFIX_BUILTINS):
                # x Speeks "word"  /  lst Lhas val  etc.
                op_tok = self._advance()
                rhs    = self._parse_primary()
                node   = BuiltinCall(func=op_tok.value, args=[node, rhs],
    ...
    def _parse_primary(self):
        ...
        if tok.type == TT.STR_TEMPLATE:
            self._advance()
            return self._parse_template_str(tok)
        ...
        # fxn call inside expression: fxn add <a, b>  or  fxn dog.speak
        if tok.type == TT.FXN:
            self._advance()
            ...
            if self._check(TT.DOT):
                self._advance()
                method_tok = self._expect(TT.IDENT)
                obj  = name
                name = method_tok.value
            args = []
            if self._check(TT.LT_OP):
        ...
        if tok.type == TT.IDENT or tok.type == TT.SELF:
            self._advance()
            name = tok.value
